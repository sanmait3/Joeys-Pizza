...
var db = monk('localhost:27017/joeyspizza');
...
router.get('/showCart/:id', function (req, res) {
  ...
  collection.find({ user_id: req.params.id }, function (err, cart) {
    ...
    {
      console.log("In loop" + cart[0].items)
    	for(cart1 of cart[0].items)
    	{
    		finalPrice+=cart1['totalprice'];
    	}
    }
    collection.update({user_id: req.params.id},
        { $set:
            {
